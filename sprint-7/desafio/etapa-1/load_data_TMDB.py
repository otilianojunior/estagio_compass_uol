...
def lambda_handler(event, context):
    ...
    def consulta_genero_data(media_type, start_year, end_year, genres='18,10749', page=1):
        try:
...
# Carrega o elenco das séries
    ...
    def save_json(media_type, start_year, end_year, genres, filename_prefix):
        try:
            page = 1
            results = []

            while True:
                response = consulta_genero_data(media_type, start_year, end_year, genres, page)
                if not response or 'results' not in response:
                    break

                filtered_results = filtro_consulta(response['results'])

                if media_type == 'tv':
                    # Adiciona o cast para cada série antes de salvar
                    for series in filtered_results:
                        series_id = series['id']
                        series['cast'] = get_elenco_serie(series_id)
                ...
                if len(results) >= 100:
                    ...
                    save_s3(results[:100], filename, media_type)
                    results = results[100:]
    ...
    try:
        ...
        save_json('movie', 2019, 2023, genres_dict['drama_romance'], 'movie')

        # Busca séries de drama e romance de 1980 a 2024
        save_json('tv', 1980, 2024, genres_dict['drama_romance'], 'serie')
